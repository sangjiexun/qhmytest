 ...
 * 
 * <p>标题:XlsxExcel</p>
 * <p>描述:支持2007 2010格式的excel</p>
 * <p>组织:河北科曼信息技术有限公司</p>
 * @author Administrator 鲁震
 * @date 2017年8月16日 下午11:12:11
 */
public class XlsxExcel extends ExcelBase{
	...
	public List<PageData> getList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			XSSFCell stuNameCell = row.getCell(0);//学生名称
			XSSFCell stuNumberCell = row.getCell(1);//学号
			XSSFCell cardNoCell = row.getCell(2);//身份证号
			XSSFCell gradeCell = row.getCell(3);//入学年份
			XSSFCell classTypeCell = row.getCell(4);//班型
			XSSFCell costCell = row.getCell(5);//应收金额
...
//			}
	...
	public List<PageData> getPartSchoolList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			XSSFCell SCHOOLNAME = row.getCell(0);//合作学校名称
			XSSFCell DINGJIN = row.getCell(1);//定金
...
//			XSSFCell itemNameCell = row.getCell(0);//缴费项目名称
			XSSFCell stuNameCell = row.getCell(0);//学生名称
			XSSFCell stuNumberCell = row.getCell(1);//学号
			XSSFCell cardNoCell = row.getCell(2);//身份证号
			XSSFCell totalMoneyCell = row.getCell(3);//缴费总金额
			XSSFCell cashCell = row.getCell(4);//现金
			XSSFCell bankCardNoCell = row.getCell(5);//银行卡号
			XSSFCell bankCardMoneyCell = row.getCell(6);//银行卡金额
			XSSFCell weChatCell = row.getCell(7);//微信
			XSSFCell alipayCell = row.getCell(8);//支付宝
			XSSFCell ttCell = row.getCell(9);//电汇
			data = new PageData();
...
//			}
	...
	public List<PageData> getPayRecordList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			row = sheet.getRow(rowNum);
			XSSFCell SHENFENZHENGHAO = row.getCell(0);//身份证号
			XSSFCell PAY_MODE = row.getCell(1);//费用类型
			XSSFCell PAY_MONEY = row.getCell(2);//费用金额
			data = new PageData();
	...
	public List<PageData> getjffbList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			XSSFCell stuNumberCell = row.getCell(1);//学号
			XSSFCell cardNoCell = row.getCell(2);//身份证号
			XSSFCell costCell = row.getCell(3);//费用
			...
			if (stuNameCell!=null) {
				stuNameCell.setCellType(Cell.CELL_TYPE_STRING);
				data.put("stuName", stuNameCell.getStringCellValue());
			}else {
				data.put("stuName", "");
			}
			if (stuNumberCell!=null) {
				stuNumberCell.setCellType(Cell.CELL_TYPE_STRING);
				data.put("stuNumber", stuNumberCell.getStringCellValue());
			}else {
				data.put("stuNumber", "");
			}
			if (cardNoCell!=null) {
				cardNoCell.setCellType(Cell.CELL_TYPE_STRING);
				data.put("cardNo", cardNoCell.getStringCellValue());
			}else {
				data.put("cardNo", "");
			}
			if (costCell!=null) {
				costCell.setCellType(Cell.CELL_TYPE_STRING);
				data.put("cost", costCell.getStringCellValue());
			}else {
				data.put("cost", "");
			}
	...
	public List<PageData> getStuInfoList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			XSSFCell SHENFENZHENGHAO = row.getCell(0);//身份证号
			XSSFCell XINGMING = row.getCell(1);//姓名
			/*HSSFCell OLD_BANJI = row.getCell(2);//原班级
*/			XSSFCell SHOUJI = row.getCell(2);//手机
			XSSFCell XUEHAO = row.getCell(3);//学号
			XSSFCell RXNIANFEN = row.getCell(4);//入学年份
			XSSFCell BANJI_TYPE = row.getCell(5);//班型
			XSSFCell CENGCI = row.getCell(6);//层次
			XSSFCell WHKXUEXIAO = row.getCell(7);//文化课学校
			XSSFCell YJ_NIANFEN = row.getCell(8);//预交年份
			XSSFCell YJ_BANJI_TYPE = row.getCell(9);//预交班型
			XSSFCell JIATINGZHUZHI = row.getCell(10);//家庭地址		
			XSSFCell YOUZHENGBIANMA= row.getCell(11);//邮政编码
			XSSFCell ONE_JHR = row.getCell(12);//第一监护人
			XSSFCell ONE_JHRGX = row.getCell(13);//第一监护人家庭关系
			XSSFCell ONE_JHRDH = row.getCell(14);//第一监护人联系电话
			XSSFCell TWO_JHR = row.getCell(15);//第二监护人
			XSSFCell TWO_JHRGX = row.getCell(16);//第二监护人家庭关系
			XSSFCell TWO_JHRDH = row.getCell(17);//第二监护人联系电话			
			XSSFCell BINGSHI = row.getCell(18);//病史
			XSSFCell LJQHTJ = row.getCell(19);//了解强化的途径
			XSSFCell TUIJIANREN = row.getCell(20);//推荐人
			XSSFCell GYRXSJ = row.getCell(21);//高一入学时间
			XSSFCell GZ_BANJI = row.getCell(22);//高中班级
			XSSFCell WENLIKE = row.getCell(23);//文理科
			XSSFCell BZRSCHOOL = row.getCell(24);//学校班主任
			XSSFCell BZRPHONE = row.getCell(25);//班主任电话
			XSSFCell STARTMEISHU = row.getCell(26);//自何时学习美术
			XSSFCell KEMU = row.getCell(27);//已学过科目
			XSSFCell LKFS = row.getCell(28);//联考分数
			XSSFCell XK_MARK = row.getCell(29);//校考成绩
			XSSFCell KSNUMBER = row.getCell(30);//考生号
			XSSFCell REMARKS1 = row.getCell(31);//备注1
			XSSFCell REMARKS2 = row.getCell(32);//备注2
			XSSFCell REMARKS3 = row.getCell(33);//备注3
			...
			/*if (OLD_BANJI!=null) {
				OLD_BANJI.setCellType(Cell.CELL_TYPE_STRING);
				data.put("OLD_BANJI", OLD_BANJI.getStringCellValue().trim());
			}else {
				data.put("OLD_BANJI", "");
			}*/
			if (REMARKS1!=null) {
	...
	public List<PageData> getStuInfoCJList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			row = sheet.getRow(rowNum);
			XSSFCell SHENFENZHENGHAO = row.getCell(0);//身份证号
			XSSFCell XUEHAO = row.getCell(1);//学号
			XSSFCell XINGMING = row.getCell(2);//姓名
			XSSFCell ZHENGZHIMIANMAO = row.getCell(3);//政治面貌
			XSSFCell SHOUJI = row.getCell(4);//联系方式
			XSSFCell KAOSHENGHAO = row.getCell(5);//考生号
			XSSFCell ZAIXUEZT = row.getCell(6);//在学状态
			XSSFCell DEPARTMEN_ID = row.getCell(7);//院校专业
			XSSFCell NIANJI = row.getCell(8);//入学年份
			XSSFCell BANJI = row.getCell(9);//班级
			XSSFCell CENGCI = row.getCell(10);//层次
			XSSFCell PICI = row.getCell(11);//批次		
			XSSFCell XUEJI = row.getCell(12);//学籍类型
			XSSFCell SHENGXUEBIAOSHI = row.getCell(13);//升学标识
			XSSFCell KELEI = row.getCell(14);//科类
			XSSFCell LUQUZHUANYE = row.getCell(15);//录取专业			
			XSSFCell MINZU = row.getCell(16);//民族
			XSSFCell YOUZHENGBIANMA = row.getCell(17);//邮政编码
			XSSFCell JIATINGZHUZHI = row.getCell(18);//家庭地址
			XSSFCell JTCYGX = row.getCell(19);//家庭成员关系
			XSSFCell JTCYXM = row.getCell(20);//家庭成员姓名
			XSSFCell JTCYLXDH = row.getCell(21);//家庭成员联系电话
			XSSFCell HUKOUXINGZHI = row.getCell(22);//户口性质
			XSSFCell JKZK = row.getCell(23);//健康状况
			XSSFCell KAOSHENGTECHANG = row.getCell(24);//考生特长
			XSSFCell SHIFOUPINKUN = row.getCell(25);//是否贫困
			XSSFCell ZXMC = row.getCell(26);//中学名称
			XSSFCell TOUDANGCHENGJI = row.getCell(27);//投档成绩
			XSSFCell XUEZHI = row.getCell(28);//学制
			XSSFCell JIHUAXINGZHI = row.getCell(29);//计划性质
			XSSFCell XSLX = row.getCell(30);//学生类型
			XSSFCell ZCXJSJ = row.getCell(31);//注册学籍时间
			
			
			XSSFCell REMARKS1 = row.getCell(32);//备注1
			XSSFCell REMARKS2 = row.getCell(33);//备注2
			XSSFCell REMARKS3 = row.getCell(34);//备注3
			XSSFCell REMARKS4 = row.getCell(35);//备注4
			XSSFCell REMARKS5 = row.getCell(36);//备注5
			XSSFCell REMARKS6 = row.getCell(37);//备注6
			XSSFCell REMARKS7 = row.getCell(38);//备注7
			XSSFCell REMARKS8 = row.getCell(39);//备注8
			XSSFCell TESHUBIAOJI = row.getCell(40);//特殊标记
	...
	public List<PageData> getTeacherInfoList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			row = sheet.getRow(rowNum);
			XSSFCell XINGMING = row == null ? null :row.getCell(0);//姓名
			XSSFCell SHENFENZHENGHAO = row == null ? null :row.getCell(1);//身份证号
			XSSFCell XINGBIE = row == null ? null :row.getCell(2);//性别
			XSSFCell SHOUJI = row == null ? null :row.getCell(3);//联系方式
			XSSFCell TEACHER_NO = row == null ? null :row.getCell(4);//教职工编码
			XSSFCell DEPARTMENTNAME = row == null ? null :row.getCell(5);//教职工编码
	...
	public List<PageData> getClassInfoList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			row = sheet.getRow(rowNum);
			XSSFCell grade = row == null ? null :row.getCell(0);//入学年份
			XSSFCell departmentId = row == null ? null :row.getCell(1);//院校专业编码
			XSSFCell className = row == null ? null :row.getCell(2);//班级名称
			XSSFCell userName = row == null ? null :row.getCell(3);//班主任名称
	...
	public List<PageData> getDormInfoList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			XSSFCell SHENFENZHENGHAO = row.getCell(0);//身份证号
			XSSFCell XINGMING = row.getCell(1);//姓名
			XSSFCell XINGBIE = row.getCell(2);//性别
			XSSFCell DORMTYPE = row.getCell(3);//宿舍类型
			XSSFCell XIAOQU = row.getCell(4);//校区
			XSSFCell SUSHELOU = row.getCell(5);//宿舍楼
			XSSFCell LOUCENG = row.getCell(6);//楼层
			XSSFCell FANGJIAN = row.getCell(7);//房间
			XSSFCell CHUANGHAO = row.getCell(8);//床号
			XSSFCell BANXING = row.getCell(9);//班型
			XSSFCell RUXUENIANFEN = row.getCell(10);//入学年份
	...
	public List<PageData> getDeptList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			row = sheet.getRow(rowNum);
			XSSFCell deptName = row.getCell(0);//组织名称
			XSSFCell deptNo = row.getCell(1);//编码
			XSSFCell leibie = row.getCell(2);//类别
			XSSFCell parentDeptNo = row.getCell(3);//上级组织编码
			XSSFCell standardCode = row.getCell(4);//标准代码
	...
	public List<PageData> getCJDeptList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			XSSFCell leibie = row.getCell(2);//类别
			XSSFCell parentDeptNo = row.getCell(3);//上级组织编码
			XSSFCell XSLX = row.getCell(4);//学生类型
			XSSFCell standardCode = row.getCell(5);//标准代码
	...
	public List<PageData> getNationList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			row = sheet.getRow(rowNum);
			XSSFCell nationName = row.getCell(0);//民族名称
			data = new PageData();
			if (nationName!=null) {
	...
	public List<PageData> getJianMianList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			row = sheet.getRow(rowNum);
			XSSFCell shenFenZhengHao = row.getCell(0);//身份证号
			XSSFCell jianMianJine = row.getCell(1);//减免金额
			data = new PageData();
			if (shenFenZhengHao!=null) {
				shenFenZhengHao.setCellType(Cell.CELL_TYPE_STRING);
				data.put("shenFenZhengHao", shenFenZhengHao.getStringCellValue().trim().toUpperCase());
	...
	public List<PageData> getStuLoanList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			...
			XSSFCell SHENFENZHENGHAO = row.getCell(0);//身份证号
			XSSFCell LOAN = row.getCell(1);//贷款金额
	...
	public List<PageData> getStuSourceList() {
		...
		for (int rowNum = 1; rowNum <= sheet.getLastRowNum(); rowNum++) {
			row = sheet.getRow(rowNum);
			XSSFCell sutSourceName = row.getCell(0);//学生来源名称
			data = new PageData();
			if (sutSourceName!=null) {
				sutSourceName.setCellType(Cell.CELL_TYPE_STRING);
				data.put("stuSourceName", sutSourceName.getStringCellValue().trim());
			}else {
