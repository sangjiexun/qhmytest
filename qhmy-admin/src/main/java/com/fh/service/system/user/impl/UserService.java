package com.fh.service.system.user.impl;

import java.util.List;

import javax.annotation.Resource;

import org.springframework.stereotype.Service;

import com.fh.dao.DaoSupport;
import com.fh.entity.Page;
import com.fh.entity.system.User;
import com.fh.service.system.user.UserManager;
import com.fh.util.PageData;


/** 系统用户
 * @author zhoudibo
 * 修改时间：2015.11.2
 */
@Service("userService")
public class UserService implements UserManager{

	@Resource(name = "daoSupport")
	private DaoSupport dao;
	
	/**登录判断
	 * @param pd
	 * @return
	 * @throws Exception
	 */
	@Override
	public PageData getUserByNameAndPwd(PageData pd)throws Exception{
		return (PageData)dao.findForObject("UserMapper.getUserInfo", pd);
	}
	
	@Override
	public PageData getDepartmentSchool(PageData pd)throws Exception{
		return (PageData)dao.findForObject("UserMapper.getDepartmentSchool", pd);
	}
	
	/**更新登录时间
	 * @param pd
	 * @throws Exception
	 */
	@Override
	public void updateLastLogin(PageData pd)throws Exception{
		dao.update("UserMapper.updateLastLogin", pd);
	}
	
	/**通过用户ID获取用户信息和角色信息
	 * @param USER_ID
	 * @return
	 * @throws Exception
	 */
	@Override
	public User getUserAndRoleById(String USER_ID) throws Exception {
		return (User) dao.findForObject("UserMapper.getUserAndRoleById", USER_ID);
	}
	
	/**通过USERNAEME获取数据
	 * @param pd
	 * @return
	 * @throws Exception
	 */
	@Override
	public PageData findByUsername(PageData pd)throws Exception{
		return (PageData)dao.findForObject("UserMapper.findByUsername", pd);
	}
	
	/**列出某角色下的所有用户
	 * @param pd
	 * @return
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> listAllUserByRoldId(PageData pd) throws Exception {
		return (List<PageData>) dao.findForList("UserMapper.listAllUserByRoldId", pd);
		
	}
	
	/**保存用户IP
	 * @param pd
	 * @throws Exception
	 */
	@Override
	public void saveIP(PageData pd)throws Exception{
		dao.update("UserMapper.saveIP", pd);
	}
	
	/**用户列表
	 * @param page
	 * @return
	 * @throws Exception
	 */
	@Override
	@SuppressWarnings("unchecked")
	public List<PageData> listUsers(Page page)throws Exception{
		return (List<PageData>) dao.findForList("UserMapper.userlistPage", page);
	}
	
	/**通过邮箱获取数据
	 * @param pd
	 * @return
	 * @throws Exception
	 */
	@Override
	public PageData findByUE(PageData pd)throws Exception{
		return (PageData)dao.findForObject("UserMapper.findByUE", pd);
	}
	
	/**通过编号获取数据
	 * @param pd
	 * @return
	 * @throws Exception
	 */
	@Override
	public PageData findByUN(PageData pd)throws Exception{
		return (PageData)dao.findForObject("UserMapper.findByUN", pd);
	}
	
	/**通过id获取数据
	 * @param pd
	 * @return
	 * @throws Exception
	 */
	@Override
	public PageData findById(PageData pd)throws Exception{
		return (PageData)dao.findForObject("UserMapper.findById", pd);
	}
	
	/**保存用户
	 * @param pd
	 * @throws Exception
	 */
	@Override
	public void saveU(PageData pd)throws Exception{
		dao.save("UserMapper.saveU", pd);
	}
	 
	/**修改用户
	 * @param pd
	 * @throws Exception
	 */
	@Override
	public void editU(PageData pd)throws Exception{
		dao.update("UserMapper.editU", pd);
	}
	
	/**删除用户
	 * @param pd
	 * @throws Exception
	 */
	@Override
	public void deleteU(PageData pd)throws Exception{
		dao.delete("UserMapper.deleteU", pd);
	}
	
	/**批量删除用户
	 * @param USER_IDS
	 * @throws Exception
	 */
	@Override
	public void deleteAllU(String[] USER_IDS)throws Exception{
		dao.delete("UserMapper.deleteAllU", USER_IDS);
	}
	
	/**用户列表(全部)
	 * @param USER_IDS
	 * @throws Exception
	 */
	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> listAllUser(PageData pd)throws Exception{
		return (List<PageData>) dao.findForList("UserMapper.listAllUser", pd);
	}
	
	/**获取总数
	 * @param pd
	 * @throws Exception
	 */
	@Override
	public PageData getUserCount(String value)throws Exception{
		return (PageData)dao.findForObject("UserMapper.getUserCount", value);
	}

	/**通过身份证号获取数据
	 * @param pd
	 * @return
	 * @throws Exception
	 */
	
	
	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> findByUSER_IDCARD(PageData pd) throws Exception {
		// TODO Auto-generated method stub
		return (List<PageData>)dao.findForList("UserMapper.findByUSER_IDCARD", pd);
	}
	
	
	
	
	/**通过劳务人员身份证号获取数据
	 * @param pd
	 * @return
	 * @throws Exception
	 */
	
	
	@Override
	public PageData findByLW_IDCARD(PageData pd) throws Exception {
		// TODO Auto-generated method stub
		return (PageData)dao.findForObject("UserMapper.findByLW_IDCARD", pd);
	}
	
	

	@Override
	public List<User> findAll1() throws Exception {
		// TODO Auto-generated method stub
		return (List<User>)dao.findAll1("UserMapper.findAll");
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> findAllLabour1(PageData pd) throws Exception {
		// TODO Auto-generated method stub
		return (List<PageData>) dao.findForList("UserMapper.findAllLabour1", pd);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> findAllLabour(Page page) throws Exception {
		// TODO Auto-generated method stub
		return (List<PageData>) dao.findForList("UserMapper.findAllLabourlistPage", page);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> findAttendance(PageData pd) throws Exception {
		// TODO Auto-generated method stub
		return (List<PageData>) dao.findForList("UserMapper.getAttendance", pd);
	}

	@Override
	public List<PageData> queryUserOrgAndProject(PageData pd) throws Exception {
		return (List<PageData>) dao.findForList("UserMapper.queryUserOrgAndProject", pd);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> queryUserOrgAndProjectGly(PageData pd)
			throws Exception {
		return (List<PageData>) dao.findForList("UserMapper.queryUserOrgAndProjectGly", pd);
	}
	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> getUser(PageData pd) throws Exception {
		// TODO Auto-generated method stub
		return (List<PageData>) dao.findForList("UserMapper.getUser", pd);
	}

	@SuppressWarnings("unchecked")
	@Override
	public List<PageData> userlistPage(Page page) throws Exception {
		return (List<PageData>) dao.findForList("UserMapper.userlistPage", page);
	}

	@Override
	public void updateU(PageData pd) throws Exception {
		dao.update("UserMapper.updateU", pd);
	}

	@Override
	public PageData findUserById(PageData pd) throws Exception {
		// TODO Auto-generated method stub
		return (PageData)dao.findForObject("UserMapper.findUserById", pd);
	}

	@Override
	public void updateUserStatus(PageData pageData) throws Exception {
		// TODO Auto-generated method stub
		dao.update("UserMapper.updateUserStatusList", pageData);
	}

	@Override
	public void updatePwd(PageData pageData) throws Exception {
		// TODO Auto-generated method stub
		dao.update("UserMapper.updatePwd", pageData);
	}

	@Override
	public void saveUserRole(PageData pageData) throws Exception {
		// TODO Auto-generated method stub
		dao.update("UserMapper.saveUserRole", pageData);
	}

	@Override
	public void saveNewPwd(PageData pd) throws Exception {
		dao.update("UserMapper.saveNewPwd", pd);
	}

	@Override
	public PageData usernameIdentify(PageData pd) throws Exception {
		return (PageData)dao.findForObject("UserMapper.usernameIdentify", pd);
	}
	@Override
	public void delUser(PageData pd) throws Exception {
		dao.update("UserMapper.delUser", pd);
	}
}
