...
public class IoUtil {
    ...
    public static File getFile(String tableName,String mulu,String filename) throws IOException {
        if (filename == null || filename.isEmpty())
            return null;
        String name = filename.replaceAll("/", Matcher.quoteReplacement(File.separator));
        File f = new File(UploadConst.uploadPathMap.get(tableName).getAbsolutePath() + File.separator + mulu + File.separator + name);
        if (!f.getParentFile().exists())
            f.getParentFile().mkdirs();
        if (!f.exists())
            f.createNewFile();
    ...
    public static File getTokenedFile(String tableName,String mulu,String key) throws IOException {
        if (key == null || key.isEmpty())
            return null;
        File f = new File(UploadConst.uploadPathMap.get(tableName).getAbsolutePath() + File.separator + mulu + File.separator + key);
        if (!f.getParentFile().exists())
            f.getParentFile().mkdirs();
        if (!f.exists())
            f.createNewFile();
    ...
    public static void storeToken(String tableName,String mulu,String key) throws IOException {
        if (key == null || key.isEmpty())
            return;
        File f = new File(UploadConst.uploadPathMap.get(tableName).getAbsolutePath() + File.separator + mulu + File.separator + key);
        if (!f.getParentFile().exists())
    ...
    public static Range parseRange(HttpServletRequest req) throws IOException {
        String range = req.getHeader(UploadConst.CONTENT_RANGE_HEADER);
    ...
    public static long streaming(InputStream in, String key, String fileName) throws IOException {
        ...
        try {
            ...
            int read = 0;
            final byte[] bytes = new byte[UploadConst.BUFFER_LENGTH];
    ...
    public static long streaming(String tableName,String mulu,InputStream in, String key, String fileName) throws IOException {
        OutputStream out = null;
        File f = getTokenedFile(tableName,mulu,key);
        try {
            out = new FileOutputStream(f);
        ...
        long length = getFile(tableName,mulu,fileName).length();
        
        if (!key.equals(fileName)) {
        	File dst = IoUtil.getFile(tableName,mulu,fileName);
            dst.delete();
            f.renameTo(dst);
