...
//			long l = listOps.rightPushAll(key, v);
	...
	public List<String> getList(String k, long start, long end) {
		try {
			...
			logger.error("获取list缓存失败key[" + KEY_PREFIX_LIST + k + ", error["
					+ t + "]");
		}
		return null;
	}
	...
	public long getListSize(String k) {
		try {
			...
			logger.error("获取list长度失败key[" + KEY_PREFIX_LIST + k + "], error["
					+ t + "]");
		}
		return 0;
	}
...
//		String key = KEY_PREFIX_LIST + k;
		try {
			...
			logger.error("移除list缓存失败key[" + KEY_PREFIX_LIST + k + ", error["
					+ t + "]");
		}
		return false;
	}
