...
(function(){
	...
	SWFProvider.prototype = {
		...
		//luzhen 解决不能动态修改option属性问题
		updateConfig:function(postVarsPerFile){
			this.config.postVarsPerFile = postVarsPerFile;	
		},
		render : function(a) {
	...
	function Main(cfg){
		...
		this.config = {
			...
			browseFileBtn : cfg.browseFileBtn || "<div>选择文件</div>",
	...
	Main.prototype = {
		...
		uploadFile : function(file, url, token, method) {
			...
			//luzhen 扩展方法,动态修改参数属性
			this.fileProvider.updateConfig(this.config.postVarsPerFile);
			//end luzhen
