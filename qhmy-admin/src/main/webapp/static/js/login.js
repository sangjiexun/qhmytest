...
(function(win, $) {
	...
	login.severCheck = function() {
		if (login.check()) {
			...
			var code = "kmykt" + loginname + ",fh," + password + "kmykt"
					+ ",fh," + $("#code").val() + ",fh," + lx;
			$.ajax({
				type : "POST",
				url : _basepath+'login_login',
				data : {
					KEYDATA : code,
					tm : new Date().getTime()
				},
				...
				success : function(data) {
					if ("success" == data.result) {
						login.saveCookie();
						window.location.href = _basepath+"main/index";
						...
						$("#code").focus();
					} else if ("noquanxian" == data.result){
						login.changeCode();
						$("#loginname").tips({
							side : 1,
							msg : "该用户用户名没有任何权限",
	...
	login.changeCode = function() {
		$("#codeImg").attr("src", _basepath+"code.do?t=" + login.genTimestamp());
	};
