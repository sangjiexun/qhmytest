...
function getHeadMsg(){
	$.ajax({
		type: "POST",
		url: locat+'/head/getList.json?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
...
function getFhsmsCount(){
	$.ajax({
		type: "POST",
		url: locat+'/head/getFhsmsCount.json?tm='+new Date().getTime(),
    	data: encodeURI(""),
		dataType:'json',
		cache: false,
		success: function(data){
...
//下线
function goOut(msg){
	window.location.href=locat+"/logout.json?msg="+msg;
}
...
function editUserH(){
	 ...
	 diag.Drag=true;
	 diag.Title ="个人资料";
	 diag.URL = locat+'/user/goEditU.json?USER_ID='+USER_ID+'&fx=head';
	 diag.Width = 469;
	 diag.Height = 469;
...
function editSys(){
	 ...
	 diag.Drag=true;
	 diag.Title ="系统设置";
	 diag.URL = locat+'/head/goSystem.json';
	 diag.Width = 600;
	 diag.Height = 526;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function fhsms(){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = locat+'/fhsms/list.json?STATUS=2';
	 diag.Width = 800;
	 diag.Height = 500;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
