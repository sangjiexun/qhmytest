...
(function(win,$,layer){
	...
	page.parse = function(file){
		...
		var tableName = "JFFB_LIST";//路径
		var fileName = file.name;//文件名
		
		var item_pkid = $("#item_pkid").val();//选择的缴费项目
		var fabuxmmc = $("#fabuxm").val(); //发布时缴费项目名称
		var pay_style = $("#pay_style").val();//发布时缴费类型
		var school_year = $("#school_year").val();//发布时学年
		var GRADE_PKID = $("#GRADE_PKID").val();//入学年份PKID
		var CLASSTYPE_PKID = $("#CLASSTYPE_PKID").val();//班型PKID
		...
		datas.push({"name":"fabuxmmc","value":fabuxmmc});
		datas.push({"name":"pay_style","value":pay_style});
		datas.push({"name":"school_year","value":school_year});
		datas.push({"name":"GRADE_PKID","value":GRADE_PKID});
		datas.push({"name":"CLASSTYPE_PKID","value":CLASSTYPE_PKID});
...
//				messagerId : "i_stream_message_container", /** 消息框的Id，当没有自定义onXXX函数，系统会显示onXXX的部分提示信息，如果没有i_stream_message_container则不显示 */
				...
				postVarsPerFile:{tableName: "JFFB_LIST", mulu: _uuid}, /** 上传文件时的其他参数  */
...
//					layer.msg('上传组件已销毁');
	...
	$("#downloadfbjf").bind('click',function(){
		var url=_basepath+"uploadFiles/template/fbjfimport.xlsx";
		window.open(url);
	});
