...
(function(win,$,layer){
	...
	//	datas.push({"name":"item_pkid","value":item_pkid});
		...
		$.ajax({
			...
			async: true,
			data : datas,
			url : _basepath + 'importstuedit/importupdate-stuinfo-parse.json',
			success : function(data) {
				if (true == data.result) {
					...
					if(data.failCount>0){
						$("#mulu").show();
						$("#mulu").attr("href",_basepath + "uploadFiles\\stuinfo\\"+data.mulu);
					}else{
						$("#mulu").hide();
					}
				}
...
//					layer.msg('上传组件已销毁');
	...
	$("#downloadtemplate").bind('click',function(){
		var url=_basepath+"uploadFiles/template/stuinfonew.xlsx";
		window.open(url);
	});
