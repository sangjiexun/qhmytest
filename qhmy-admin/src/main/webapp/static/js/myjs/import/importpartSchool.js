...
(function(win,$,layer){
	...
	page.parse = function(file){
		...
		$.ajax({
			...
			dataType : "json",
			data : datas,
			url : _basepath + 'importPartSchool/import-list-parse.json',
			success : function(data) {
				if (true == data.result) {
					...
					if(data.failCount>0){
						$("#mulu").show();
						$("#mulu").attr("href",_basepath + "uploadFiles\\partschool\\"+data.mulu);
					}else{
						$("#mulu").hide();
					}
				}
...
//				messagerId : "i_stream_message_container", /** 消息框的Id，当没有自定义onXXX函数，系统会显示onXXX的部分提示信息，如果没有i_stream_message_container则不显示 */
				...
				postVarsPerFile:{tableName: "PART_SCHOOL", mulu: page.uuid}, /** 上传文件时的其他参数  */
...
//					layer.msg('文件被选择');
	...
	$("#downloadtemplate").bind('click',function(){
		var url=_basepath+"uploadFiles/template/partschool.xlsx";
		window.open(url);
	});
