...
(function(win,$,layer){
	...
	//	datas.push({"name":"item_pkid","value":item_pkid});
		var college_pkid=$("#colleagename").find("option:selected").val();
		if(college_pkid==''){
			layer.msg("请选择学校!");
			return false;
		}
		$.ajax({
			...
			dataType : "json",
			data : datas,
			url : _basepath + 'importteacher/import-teacherinfo-parse.json?college_pkid='+college_pkid,
			success : function(data) {
				if (true == data.result) {
					...
					if(data.failCount>0){
						$("#mulu").show();
						$("#mulu").attr("href",_basepath + "uploadFiles\\teacher\\"+data.mulu);
					}else{
						$("#mulu").hide();
					}
				}
...
//				messagerId : "i_stream_message_container", /** 消息框的Id，当没有自定义onXXX函数，系统会显示onXXX的部分提示信息，如果没有i_stream_message_container则不显示 */
				...
				postVarsPerFile:{tableName: "TEACHER_INFO", mulu: _uuid}, /** 上传文件时的其他参数  */
...
//					layer.msg('上传组件已销毁');
	...
	$("#downloadtemplate").bind('click',function(){
		var url=_basepath+"uploadFiles/template/teacher.xlsx";
		window.open(url);
	});
