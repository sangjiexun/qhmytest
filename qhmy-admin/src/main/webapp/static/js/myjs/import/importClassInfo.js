...
(function(win,$,layer){
	...
	page.parse = function(file){
		...
		$.ajax({
			...
			dataType : "json",
			data : datas,
			url : _basepath + 'importclass/import-classinfo-parse.json',
			success : function(data) {
				if (true == data.result) {
					$("#totalCount").html('总记录数:'+data.totalCount);
					$("#successCount").html('成功更新记录数:'+data.successCount);
					$("#failCount").html('失败记录数:'+data.failCount);
					if(data.failCount>0){
						$("#mulu").show();
						$("#mulu").attr("href",_basepath + "uploadFiles\\classinfo\\"+data.mulu);
					}else{
						$("#mulu").hide();
					}
				}
...
//				messagerId : "i_stream_message_container", /** 消息框的Id，当没有自定义onXXX函数，系统会显示onXXX的部分提示信息，如果没有i_stream_message_container则不显示 */
				...
				postVarsPerFile:{tableName: "CLASS_INFO", mulu: _uuid}, /** 上传文件时的其他参数  */
...
//					layer.msg('上传组件已销毁');
	...
	$("#downloadtemplate").bind('click',function(){
		var url=_basepath+"uploadFiles/template/classinfotemplate.xlsx";
		window.open(url);
	});
