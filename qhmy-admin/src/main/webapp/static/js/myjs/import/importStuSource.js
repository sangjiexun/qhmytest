...
(function(win,$,layer){
	...
	//	datas.push({"name":"item_pkid","value":item_pkid});
		...
		$.ajax({
			...
			dataType : "json",
			data : datas,
			url : _basepath + 'importStuSource/import-stuSource-parse.json',
			success : function(data) {
				if (true == data.result) {
					...
					if(data.failCount>0){
						$("#mulu").show();
						$("#mulu").attr("href",_basepath + "uploadFiles\\stuSource\\"+data.mulu);
					}else{
						$("#mulu").hide();
					}
				}
...
//				messagerId : "i_stream_message_container", /** 消息框的Id，当没有自定义onXXX函数，系统会显示onXXX的部分提示信息，如果没有i_stream_message_container则不显示 */
				...
				postVarsPerFile:{tableName: "STUSOURCE", mulu: _uuid}, /** 上传文件时的其他参数  */
...
//					BootstrapDialog.alert({title: '提示信息',message: '取消某个文件,从上传列表删除,'+file.id });
				...
				onComplete: function(file) {
					//单个文件上传完毕的响应事件 
					layer.msg('['+file.name+']:上传成功,开始分析,请稍等...');
					page.parse(file);
				},
...
//					layer.msg('上传组件已销毁');
	...
	$("#downloadtemplate").bind('click',function(){
		var url=_basepath+"uploadFiles/template/stuSource.xlsx";
		window.open(url);
	});
