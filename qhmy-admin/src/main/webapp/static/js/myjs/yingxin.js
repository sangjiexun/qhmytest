...
//			$("#checkReset").click();
	...
	yingxin.isCanClickFlowNode = function(flowNode){
		...
		for (var i = 0; i < sessionMenuListJson.length; i++) {
			if(sessionMenuListJson[i].XTMC == '报到' && flowNode == '1'){
				result = true;
				break;
			}else if(sessionMenuListJson[i].XTMC == '办理绿色通道' && flowNode == '2'){
				result = true;
				break;
			}else if(sessionMenuListJson[i].XTMC == '缴费' && flowNode == '3'){
				result = true;
				break;
			}else if(sessionMenuListJson[i].XTMC == '领取物品' && flowNode == '4'){
				result = true;
				break;
			}else if(sessionMenuListJson[i].XTMC == '领宿舍钥匙入住' && flowNode == '5'){
				result = true;
				break;
			}
		}
	...
	//流程节点点击事件
	$('#lingquwupinFlow').click(function(){
...
//	if(payItemPkidArrays != '' && payItemPkidArrays.length > 0){
	...
	payBatchArrays += '<div action="" class="form-inline" style="margin-bottom: 10px;" >'+
	'<div class="form-group col-xs-5">'+
	'<label><input class="payTypeBatch" name="payTypeBatch" type="radio" value="" checked /></label><span>银行卡</span>'+//update by ccc
...
$("#payPrintBatchButton").click(function(){
	...
	if(yingxin.fromSchool == "CAIJING"){
		payPrintBatch();
	}else if(yingxin.fromSchool == "LIGONG"){
		payPrintBatch2();
	}else if(yingxin.fromSchool == "GUANGDIAN"){
		payPrintBatch3();
	}else if(yingxin.fromSchool == "WAIGUOYU"){
		payPrintBatch4();
	}else{
		payPrintBatch3();
	}
...
var payPrintBatch = function(){
	//如果没有多选,批量打印点击无效果
	if($("#payPrintBatchButton").attr("disabled") == "disabled" && $("#payPrintBatchButton").attr("data-target") == "#mymoda2"){
			  ...
			  if(result.result == 'success'){
				  ...
				  for(var key in dataMap){
					    ...
					    for (var i = 0; i < payOrderDetailList.length - 1; i++) {
							...
							//表头-------------------------------------------------------------------------------------------------
							if(i == 0 || i % 4  == 0){//4条数据打一张单据;border:1px solid
								count ++;
								totalMoney = 0;
								count ++;//padding:10px 35px 30px;
								var DEPARTMENT_NAME = payItemList.DEPARTMENT_NAME;
								if(DEPARTMENT_NAME != null && DEPARTMENT_NAME != ''){
									if(DEPARTMENT_NAME.length > 15){
										DEPARTMENT_NAME = DEPARTMENT_NAME.substr(0,15);
									}
								}
								createDate = new Date(payItemList.CJSJ).Format("yyyy-MM-dd");
								createDateArray = createDate.split("-");
								
								htmls += '<div style="width:140mm;height:70mm;margin:0 auto;">';
								htmls += '<div><h3 style="text-align: center;margin-top:0;">'+$("#COLLEGES_NAME").val()+'收费票据</h3>'+
									'<div style="float:left;font-weight: bold;margin-left:0px;">姓名:'+payItemList.XINGMING+'-'+payItemList.NIANJI+'-'+payItemList.XUENIANZHI+'-'+DEPARTMENT_NAME+'</span></div>'+
									'<div style="float:left;font-weight: bold;margin-left:10px;">'+createDateArray[0]+'-<span style="margin-left: 0px;">'+createDateArray[1]+'-</span><span style="margin-left: 0px;">'+createDateArray[2]+'</span></div>'+
									'<div style="float:right;margin-right: 0px;font-weight: bold;">&nbsp;</div>'+//No:'+payItemList.RECEIPTNO+'
									'<div style="clear:both;"></div>'+
									'<table class="table table-bordered2 table-condensed">'+
										'<tr>'+
											'<td colspan="2">收费项目</td>'+
											'<td colspan="2">数量</td>'+
											'<td colspan="2">单价</td>'+
											'<td colspan="2">金额</td></tr>';
								isFirstCount ++;
							};
							...
							//表中-项目记录-------------------------------------------------------------------------------------------------
							htmls += '<tr>'+
//										'<td colspan="2" style="max-width:100px;">'+(payItemList.PAYITEM_CHILD == '' || payItemList.PAYITEM_CHILD == null || payItemList.PAYITEM_CHILD == 'null'  || typeof payItemList.PAYITEM_CHILD == 'undefined'
//											? payItemList.PAYITEM :(payItemList.PAYITEM + ' - ' + payItemList.PAYITEM_CHILD))+'</td>'+
										'<td colspan="2" style="max-width:100px;">'+(payItemList.PAY_STYLE_NAME == '' || payItemList.PAY_STYLE_NAME == null || payItemList.PAY_STYLE_NAME == 'null'  || typeof payItemList.PAY_STYLE_NAME == 'undefined'
											? '' : payItemList.PAY_STYLE_NAME)+'</td>'+
										'<td colspan="2">1</td>'+
										'<td colspan="2">'+(payItemList.INPUT_OUTPUT == 'XX' ? '-' : '')+payItemList.MONEY+'</td>'+
										'<td colspan="2">'+(payItemList.INPUT_OUTPUT == 'XX' ? '-' : '')+payItemList.MONEY+'</td>'+
									'</tr>';
							recordCount ++;
						  
							//表尾-------------------------------------------------------------------------------------------------
							if((i + 1) % 4  == 0){//4条数据打一张单据
								isEnd = true;
								htmls += '<tr>'+
											'<td colspan="4" class="text-left">合计金额：(大写) '+smalltoBIG(totalMoney)+'</td>'+
											'<td colspan="4" class="text-center">￥: '+totalMoney.toFixed(2)+'</td>'+
										'</tr>'+
									'</table>'+
									'<div style="font-weight: bold;float:left;">收费单位：(盖章)<span> '+$("#COLLEGES_NAME").val()+'</span></div>'+
									'<div style="font-weight: bold;float:right;margin-right: 0px;">开票人：(盖章) '+(KAIPIAOREN == '' || KAIPIAOREN == null || KAIPIAOREN == 'null'  || typeof KAIPIAOREN == 'undefined'
										? '' :KAIPIAOREN)+'</div>'+
								'</div>';
								htmls += '</div>';
								totalMoney = 0;
			  ...
			  if(result.result == 'success'){
				  ...
				  for(var key in dataMap){
					    ...
					    for (var i = 0; i < payOrderDetailList.length - 1; i++) {
							...
							//表头-------------------------------------------------------------------------------------------------
							if(i == 0 || i % 5  == 0){//4条数据打一张单据;border:1px solid
								count ++;
								totalMoney = 0;
								count ++;
								/*$.ajax({
									  type: 'POST',
									  url: _basepath+"receipt/getSeqReceiptNO.json",
									  async: false,
									  data: {
										  PKID:payItemList.PKID
									  },
									  dataType : "json",
									  success : function(result) {
								...
								//收据号：'+payOrderDetailList[payOrderDetailList.length - 1].RECEIPTNO+'
								htmls += '<div style="width:190mm;height:118mm;margin:0 auto;">';
								htmls += '<div><h3 style="text-align: center;margin:0;">收据</h3>'+
									'<div style="float:left;font-weight: bold;margin-left:50px;">'+createDateArray[0]+' 年<span style="margin-left: 20px;">'+createDateArray[1]+' 月</span><span style="margin-left: 20px;">'+createDateArray[2]+' 日</span></div>'+
									'<div style="float:right;margin-right: 0px;font-weight: bold;" id="paperRECEIPTNO'+paperCount+'"></div>'+
									'<div style="clear:both;"></div>'+
									'<table class="table table-bordered2" style="margin-top:0px;">'+
										'<tr>'+
											'<td>证件号</td>'+
											'<td>'+payItemList.SHENFENZHENGHAO+'</td>'+
											'<td>姓名</td>'+
											'<td>'+payItemList.XINGMING+'</td>'+
											'<td>批次</td>'+
											'<td>'+(typeof payItemList.PICI_NAME !='undefined' && payItemList.PICI_NAME !=null?payItemList.PICI_NAME:'')+'</td>'+
										'</tr>'+
										'<tr>'+
											'<td>学校</td>'+
											'<td>'+payItemList.XUEXIAONAME+'</td>'+
											'<td>学生类型</td>'+
											'<td>'+(typeof payItemList.CENGCI_NAME !='undefined' && payItemList.CENGCI_NAME !=null?payItemList.CENGCI_NAME:'')+'</td>'+
											'<td>专业</td>'+
											'<td>'+payItemList.DEPARTMENT_NAME+'</td>'+
										'</tr>'+
										'<tr>'+
											'<td colspan="2">收费项目</td>'+
							...
							/*$.ajax({
								  type: 'POST',
								  url: _basepath+"receipt/updatePayOrderDetailStatus.json",
								  async: true,
								  data: {
									  PKID:payItemList.PKID,
									  RECEIPTNO:RECEIPTNO
							...
							//表中-项目记录-------------------------------------------------------------------------------------------------
							htmls += '<tr>'+
										'<td colspan="2">'+(payItemList.PAYITEM_CHILD == '' || payItemList.PAYITEM_CHILD == null || payItemList.PAYITEM_CHILD == 'null'  || typeof payItemList.PAYITEM_CHILD == 'undefined'
											? payItemList.PAYITEM :(payItemList.PAYITEM + ' - ' + payItemList.PAYITEM_CHILD))+'</td>'+
										'<td colspan="2">'+payItemList.AMOUNTRECEIVABLE+'</td>'+
										'<td colspan="2">'+(payItemList.INPUT_OUTPUT == 'XX' ? '-' : '')+payItemList.MONEY+'</td>'+
									'</tr>';
							recordCount ++;
						  
							//表尾-------------------------------------------------------------------------------------------------
							if((i + 1) % 5  == 0){//4条数据打一张单据
								isEnd = true;
								htmls += '<tr>'+
											'<td>合计</td>'+
											'<td colspan="3" class="text-center">'+smalltoBIG(totalMoney)+'</td>'+
											'<td colspan="2" class="text-center">'+totalMoney.toFixed(2)+'</td>'+
										'</tr>'+
									'</table>'+
									'<div style="font-weight: bold;float:left;">单位：<span>'+$("#COLLEGES_NAME").val()+'</span></div>'+
									'<div style="font-weight: bold;float:right;margin-right: 120px;" id="paperSHOUKUANREN'+paperCount+'"></div>'+
								'</div>';
								htmls += '</div>';
								totalMoney = 0;
								recordCount = 0;
				...
				//将每张票的收据号添加到票上
				for(var i = 0; i < RECEIPTNOArray.length; i++){
					$("#paperRECEIPTNO"+i).html("收据号：" + RECEIPTNOArray[i].substr(0, RECEIPTNOArray[i].length - 1));
				}
				//将每张票的收款人添加到票上
				for(var i = 0; i < SHOUKUANRENArray.length; i++){
					var SHOUKUANRENSS = SHOUKUANRENArray[i].substr(0, SHOUKUANRENArray[i].length - 1);
					...
					$("#paperSHOUKUANREN"+i).html("收款人：" + SHOUKUANRENSS);
				}
...
$("#printBatchButton").click(function(){
	...
	if(yingxin.fromSchool == "GUANGDIAN"){
...
var printBatch3 = function(){
	//如果没有多选,批量打印点击无效果
	if($("#printBatchButton").attr("disabled") == "disabled" && $("#printBatchButton").attr("data-target") == "#mymoda2"){
		$("#printBatchButton").attr("data-target","")
		return;
	}
	var payItemPkids = yingxin.getIdSelections()+"";
	var payItemPkidsArrays = payItemPkids.split(",");
	...
	$.ajax({
		  ...
		  success : function(result) {
			  if(result.result == 'success'){
				  if(result.isHasNum == '1'){
					  layer.msg("发票号已使用完毕,请先领取发票号！"); 
					  return false;
				  }
				  var dataMap = result.dataMap;
				  if(JSON.stringify(dataMap) == "{}"){
					  layer.msg("请至少选择一条未打印、可打发票、已缴费的项目！"); 
					  return false;
				  }
				  var isEnd = false;
				  var htmls = '';
				  ...
				  var tempParam = "";
				  for(var key in dataMap){
					  var payMode = key.split('|')[1];
					  	if(payMode != null && payMode != '' && payMode != 'null'){
							if(payMode == "CASH"){
								payMode = "现金";
					    ...
					    for (var i = 0; i < payOrderDetailList.length - 1; i++) {
					    	...
					    	var payItemList = payOrderDetailList[i];
							//表头-------------------------------------------------------------------------------------------------
							if(i == 0 || i % 6  == 0){//4条数据打一张单据;border:1px solid
								
								receiptIndex++;
								$.ajax({
									  type: 'POST',
									  url: _basepath+"receipt/getinvoicestatus.json",
									  async: false,
									  data: {		
										  index:receiptIndex
									  },
									  ...
									  success : function(result) {
										  if(result.isHasNum == '1'){
											  ...
											  faPiaoHao = result.pd.INVOICE_NO;
											  invoiceNumList+=(faPiaoHao+",");
										  }
									  }
							     });

								//如果isFlag是false 代表库中已经没有可使用发票
								if(isFlag == false){
									return false;
								}
								count ++;
								totalMoney = 0;
								count ++;
								htmls += '<div class="box" style="width:150mm;height:90mm;margin:0 auto;">';//
								htmls += '<div class="chart-head" style="height:17mm;">'+
											'<div>&nbsp;</div>'+
											'<p>&nbsp;</p>'+
										'</div>';
								htmls += '<ul class="faPiao" style="padding-right:5mm;">'+
											'<li>&nbsp; <span>&nbsp;</span></li>'+
											'<li>&nbsp; <span  id="paperRECEIPTNO'+paperCount+'">&nbsp;</span></li>'+
										'</ul>';//'+payItemList.RECEIPTNO+'
								htmls += '<div class="chart-body" style="margin-top:0;margin-bottom:0;">'+
											'<ul>'+
												'<li class="danWei" style="padding-left:17mm;margin-top:0;margin-bottom:0;">'+
													'&nbsp;<span></span> '+createDateArray[0]+' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span></span> '+createDateArray[1]+' &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<span></span> '+createDateArray[2]+' &nbsp;'+
												'</li>'+
												/*'<li class="clearfix"></li>'+*/
											'</ul>'+
										'</div>';
								htmls += '<table style="width:100%;border-collapse: collapse;table-layout: fixed;" class="dataTable">'+
												'<tbody>'+
													'<tr style="height:5mm;">'+
														'<td style="height:5mm;">&nbsp;</td>'+
														/*'<td class="printText"  style="height:5mm;white-space:nowrap !important;">'+payItemList.RECEIPTNO+'</td>'+*/
														'<td class="printText"  style="height:5mm;white-space:nowrap !important;">'+faPiaoHao+'</td>'+
														/*'<td class="printText"  style="height:5mm;white-space:nowrap !important;">'+result.INVOICE_NO+'</td>'+*/
														'<td style="height:5mm;">&nbsp;</td>'+
														'<td style="height:5mm;white-space:nowrap !important;">'+payItemList.XINGMING+'</td>'+
														'<td style="height:5mm;">&nbsp;</td>'+
														'<td style="height:5mm;">'+(typeof payItemList.XUEHAO !='undefined' && payItemList.XUEHAO !=null?payItemList.XUEHAO:'')+'</td>'+
														'<td style="height:5mm;">&nbsp;</td>'+
														'<td style="height:5mm;white-space:nowrap !important;">&nbsp;</td>'+//'+createDateArray[0]+'年新生费用
													'</tr>'+
													'<tr class="borNone"  style="height:5mm;">'+
														'<td>&nbsp;</td>'+
														'<td id="paperORDERCREATE_MODE'+paperCount+'" style="white-space:nowrap;">'+payMode+'</td>'+
														'<td>&nbsp;</td>'+
														'<td colspan="5" style="white-space:nowrap !important;">'+payItemList.DEPARTMENT_NAME+'</td>'+
													'</tr>'+
												'</tbody>'+
											'</table>';
								htmls += '<table style="width:100%;border-collapse: collapse;table-layout: fixed;">'+
												'<tr style="height:8mm;">'+
													'<td style="height:8mm;width: 18mm;">&nbsp;</td>'+
													'<td style="height:8mm;width: 18mm;">&nbsp;</td>'+
													'<td colspan="2" style="height:8mm;width: 45mm;">&nbsp;</td>'+
													'<td colspan="3" style="height:8mm;width: 36mm;">'+
														/*'<table style="width:100%;border-collapse: collapse;table-layout: fixed;height:8mm;">'+
															'<tr style="height:4mm;">'+
																'<td colspan="7"></td>'+
															'</tr>'+
															'<tr style="height:4mm;">'+
														...
														'</table>'+*/
													'</td>'+
													'<td style="height:8mm;width: 27mm;white-space:nowrap !important;"">&nbsp;</td>'+
												'</tr>';
								isFirstCount ++;
							};
							//将缴费明细pkid和发票号拼接
							tempParam += payItemList.PKID+"+"+faPiaoHao+",";
							if(payItemList.INPUT_OUTPUT == 'JX'){
								totalMoney += payItemList.MONEY;
							}else{
								totalMoney -= payItemList.MONEY;
							...
							if(ORDERCREATE_MODES.indexOf(payItemList.ORDERCREATE_MODE) < 0){
								ORDERCREATE_MODES += payItemList.ORDERCREATE_MODE;
								ORDERCREATE_MODES += "+";
							}
							...
							if(payItemList.KAIPIAOREN != '' && payItemList.KAIPIAOREN != null && payItemList.KAIPIAOREN != 'null'){
								if(KAIPIAORENS.indexOf(payItemList.KAIPIAOREN) < 0){
									KAIPIAORENS += payItemList.KAIPIAOREN;
									KAIPIAORENS += "/";
								}
							}else{
								KAIPIAORENS += "";
							...
							//表中-项目记录-------------------------------------------------------------------------------------------------
							htmls += '<tr style="height:6mm;">'+
										'<td style="white-space:nowrap;" style="height:6mm;">'+(payItemList.SCHOOL_YEAR_NAME == '' || payItemList.SCHOOL_YEAR_NAME == null || payItemList.SCHOOL_YEAR_NAME == 'null'  || typeof payItemList.SCHOOL_YEAR_NAME == 'undefined'
											? '' :payItemList.SCHOOL_YEAR_NAME)+'</td>'+
										'<td style="white-space:nowrap;" style="height:6mm;">'+(payItemList.PAY_STYLE_NAME == '' || payItemList.PAY_STYLE_NAME == null || payItemList.PAY_STYLE_NAME == 'null'  || typeof payItemList.PAY_STYLE_NAME == 'undefined'
											? '' :payItemList.PAY_STYLE_NAME)+'</td>'+
										'<td colspan="2" style="height:6mm;">'+payItemList.AMOUNTRECEIVABLE+'</td>'+
										'<td colspan="3" style="height:6mm;">'+
											'<table style="width:100%;border-collapse: collapse;table-layout: fixed;">'+
												'<tr>'+
													'<td style="width:5mm;">'+moneyArray[0]+'</td>'+
													'<td style="width:5mm;">'+moneyArray[1]+'</td>'+
													'<td style="width:5mm;">'+moneyArray[2]+'</td>'+
													'<td style="width:5mm;">'+moneyArray[3]+'</td>'+
													'<td style="width:5mm;">'+moneyArray[4]+'</td>'+
													'<td style="width:5mm;">'+moneyArray[5]+'</td>'+
													'<td style="width:5mm;">'+moneyArray[6]+'</td>'+
												'</tr>'+
											'</table>'+
										'</td>'+
										'<td style="white-space:nowrap;">'+(payItemList.DORMNAME == '' || payItemList.DORMNAME == null || payItemList.DORMNAME == 'null'  || typeof payItemList.DORMNAME == 'undefined'? '' :payItemList.DORMNAME)+'</td>'+
									'</tr>';
							recordCount ++;
						  
							//表尾-------------------------------------------------------------------------------------------------
							if((i + 1) % 6  == 0){//4条数据打一张单据
								isEnd = true;
								var totalMoneyArray = splitMoney(""+totalMoney+"");
								htmls += '<tr style="height:6mm;">'+
												'<td>&nbsp;</td>'+
												'<td colspan="3">'+smalltoBIG(totalMoney)+'</td>'+
												'<td colspan="3">'+
													'<table style="width:100%;border-collapse: collapse;table-layout: fixed;">'+
														'<tr>'+
															'<td style="width:5mm;">'+totalMoneyArray[0]+'</td>'+
															'<td style="width:5mm;">'+totalMoneyArray[1]+'</td>'+
															'<td style="width:5mm;">'+totalMoneyArray[2]+'</td>'+
															'<td style="width:5mm;">'+totalMoneyArray[3]+'</td>'+
															'<td style="width:5mm;">'+totalMoneyArray[4]+'</td>'+
															'<td style="width:5mm;">'+totalMoneyArray[5]+'</td>'+
															'<td style="width:5mm;">'+totalMoneyArray[6]+'</td>'+
														'</tr>'+
													'</table>'+
												'</td>'+
												'<td></td>'+
								...
								htmls += '</table>'+
										'<div class="footer" style="height:6mm;margin-bottom:0;">'+
												...
												'</li>'+
												'<li class="huoBi" style="margin-top:0;margin-bottom:0;">'+
													'&nbsp;<span id="paperKAIPIAOREN'+paperCount+'"></span>'+//'+(KAIPIAOREN == '' || KAIPIAOREN == null || KAIPIAOREN == 'null'  || typeof KAIPIAOREN == 'undefined'? '' :KAIPIAOREN)+'
												'</li>'+
												'<li class="huoBi2" style="margin-top:0;margin-bottom:0;">'+
													'&nbsp;<span  style="white-space:nowrap !important;">'+XGR+'</span>'+//id="paperSHOUKUANREN'+paperCount+'"
												'</li>'+
												/*'<li class="clearfix"></li>'+*/
											'</ul>'+
										'</div>';
								...
								KAIPIAORENArray[paperCount] = KAIPIAORENS;
								KAIPIAORENS = "";
					  ...
					  if(!isEnd){
						  ...
						  htmls += '<tr style="height:6mm;">'+
									...
									'</tr>';
						  htmls += '</table>'+
								'<div class="footer" style="margin-bottom:0;height:6mm;">'+
										'<ul style="margin:0;">'+
										'<li class="danWei" style="margin-top:0;margin-bottom:0;">'+
											'&nbsp;'+
										'</li>'+
				...
				$("#batchPrintArea2GUANGDIAN").html(htmls);
				//将每张票的收据号添加到票上
				/*for(var i = 0; i < RECEIPTNOArray.length; i++){
					$("#paperRECEIPTNO"+i).html(RECEIPTNOArray[i].substr(0, RECEIPTNOArray[i].length - 1));
				}*/
				...
				for(var i = 0; i < KAIPIAORENArray.length; i++){
					var KAIPIAORENSS = KAIPIAORENArray[i].substr(0, KAIPIAORENArray[i].length - 1);
					var KAIPIAORENSSArray = KAIPIAORENSS.split("/");
					if(KAIPIAORENSSArray.length > 2){
						KAIPIAORENSS = KAIPIAORENSSArray[0]+"/"+KAIPIAORENSSArray[1];
					}
					$("#paperKAIPIAOREN"+i).html(KAIPIAORENSS);
				}
				$.ajax({
					  type: 'POST',
					  url: _basepath+"receipt/updateinvoicestatus.json",
					  async: false,
					  data: {		
						  invoiceNumList:invoiceNumList,
						  tempParam : tempParam
					  },
					  dataType : "json",
					  success : function(result) {
				...
				//打印区域
				$("#batchPrintAreaGUANGDIAN").printArea();
	...
	$.ajax({
		  ...
		  success : function(result) {
			  if(result.result == 'success'){
				  var dataMap = result.dataMap;
				  if(JSON.stringify(dataMap) == "{}"){
					  layer.msg("请至少选择一条未打印、可打收据、已缴费的项目！"); 
					  return false;
				  }
				  var isEnd = false;
				  var htmls = '';
				 ...
				 var createDate = result.sysDay;
				  ...
				  for(var key in dataMap){
					    ...
					    for (var i = 0; i < payOrderDetailList.length - 1; i++) {
							...
							//表头-------------------------------------------------------------------------------------------------
							if(i == 0 || i % 8  == 0){//4条数据打一张单据;border:1px solid
								count ++;
								totalMoney = 0;
								count ++;
								
								htmls += '<div class="box" style="width:200mm;height:118mm;margin:0 auto;">';//
								htmls += '<div class="chart-head" style="height:10mm;">'+
											'&nbsp;'+
										'</div>';
								htmls += '<div class="chart-body" style="margin-top:0;margin-bottom:0;">'+
											'<ul>'+
												'<li class="danWei" style="padding-left:78mm;margin-top:0;margin-bottom:0;">'+
													''+createDateArray[0]+'&nbsp;<span></span>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+createDateArray[1]+'<span></span>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; '+createDateArray[2]+' &nbsp;'+
												'</li>'+
												'<li class="number">&nbsp;<span>&nbsp;</span></li>'+
											'</ul>'+
								...
								htmls += '<table style="width:100%;border-collapse: collapse;table-layout: fixed;" class="dataTable">'+
												'<tbody>'+
													'<tr style="height:8mm;">'+
														'<td style="height:8mm;width:21mm;">&nbsp;</td>'+
														'<td class="printText"  style="height:8mm;width:21mm;white-space:nowrap !important;"">'+payItemList.RECEIPTNO+'</td>'+
														'<td style="height:8mm;width:25mm;">&nbsp;</td>'+
														'<td style="height:8mm;width:22mm;white-space:nowrap !important;">'+payItemList.XINGMING+'</td>'+
														'<td style="height:8mm;width:16mm;">&nbsp;</td>'+
														'<td style="height:8mm;width:29mm;white-space:nowrap !important;">'+(typeof payItemList.XUEHAO !='undefined' && payItemList.XUEHAO !=null?payItemList.XUEHAO:'')+'</td>'+
														'<td style="height:8mm;width:18mm;">&nbsp;</td>'+
														'<td style="height:8mm;width:45mm;white-space:nowrap !important;">'+payItemList.DEPARTMENT_NAME+'</td>'+
													'</tr>'+
													'<tr class="borNone" style="height:8mm;">'+
														'<td style="width:21mm;">&nbsp;</td>'+
														'<td id="paperORDERCREATE_MODE'+paperCount+'" style="width:21mm;white-space:nowrap;">'+payMode+'</td>'+
														'<td style="width:25mm;">&nbsp;</td>'+
														'<td colspan="5" style="white-space:nowrap !important;">'+createDateArray[0]+'年新生费用</td>'+
													'</tr>'+
												'</tbody>'+
											'</table>';
								htmls += '<table style="width:100%;border-collapse: collapse;table-layout: fixed;">'+
												'<tr style="height:15mm;">'+
													'<td style="height:15mm;width: 21mm;">&nbsp;</td>'+
													'<td style="height:15mm;width: 21mm;">&nbsp;</td>'+
													'<td colspan="2" style="height:15mm;width: 64mm;">&nbsp;</td>'+
													'<td colspan="3" style="height:15mm;width: 54mm;">'+
													'</td>'+
													'<td style="height:15mm;width: 37mm;white-space:nowrap !important;"">&nbsp;</td>'+
												'</tr>';
								isFirstCount ++;
							};
							...
							htmls += '<tr style="height:7mm;">'+
										'<td style="white-space:nowrap;" style="height:7mm;">'+(payItemList.SCHOOL_YEAR_NAME == '' || payItemList.SCHOOL_YEAR_NAME == null || payItemList.SCHOOL_YEAR_NAME == 'null'  || typeof payItemList.SCHOOL_YEAR_NAME == 'undefined'
											? '' :payItemList.SCHOOL_YEAR_NAME)+'</td>'+
										'<td style="white-space:nowrap;" style="height:7mm;">'+(payItemList.PAY_STYLE_NAME == '' || payItemList.PAY_STYLE_NAME == null || payItemList.PAY_STYLE_NAME == 'null'  || typeof payItemList.PAY_STYLE_NAME == 'undefined'
											? '' :payItemList.PAY_STYLE_NAME)+'</td>'+
										'<td colspan="2" style="height:7mm;">'+payItemList.AMOUNTRECEIVABLE+'</td>'+
										'<td colspan="3" style="height:7mm;">'+
											'<table style="width:100%;border-collapse: collapse;table-layout: fixed;">'+
												'<tr>'+
													'<td style="width:7mm;">'+moneyArray[0]+'</td>'+
													'<td style="width:7mm;">'+moneyArray[1]+'</td>'+
													'<td style="width:7mm;">'+moneyArray[2]+'</td>'+
													'<td style="width:8mm;">'+moneyArray[3]+'</td>'+
													'<td style="width:7mm;">'+moneyArray[4]+'</td>'+
													'<td style="width:7mm;">'+moneyArray[5]+'</td>'+
													'<td style="width:7mm;">'+moneyArray[6]+'</td>'+
												'</tr>'+
											'</table>'+
										'</td>'+
										'<td style="white-space:nowrap;">'+(payItemList.DORMNAME == '' || payItemList.DORMNAME == null || payItemList.DORMNAME == 'null'  || typeof payItemList.DORMNAME == 'undefined'? '' :payItemList.DORMNAME)+'</td>'+
							...
							//表尾-------------------------------------------------------------------------------------------------
							if((i + 1) % 8  == 0){//4条数据打一张单据
								isEnd = true;
								var totalMoneyArray = splitMoney(""+totalMoney+"");
								htmls += '<tr style="height:8mm;">'+
												'<td>&nbsp;</td>'+
												'<td colspan="3">'+smalltoBIG(totalMoney)+'</td>'+
												'<td colspan="3">'+
													'<table style="width:100%;border-collapse: collapse;table-layout: fixed;">'+
														'<tr>'+
															'<td style="width:8mm;">'+totalMoneyArray[0]+'</td>'+
															'<td style="width:8mm;">'+totalMoneyArray[1]+'</td>'+
															'<td style="width:8mm;">'+totalMoneyArray[2]+'</td>'+
															'<td style="width:8mm;">'+totalMoneyArray[3]+'</td>'+
															'<td style="width:8mm;">'+totalMoneyArray[4]+'</td>'+
															'<td style="width:8mm;">'+totalMoneyArray[5]+'</td>'+
															'<td style="width:8mm;">'+totalMoneyArray[6]+'</td>'+
														'</tr>'+
													'</table>'+
												'</td>'+
												'<td></td>'+
								...
								htmls += '</table>'+
										'<div class="footer" style="height:6mm;margin-bottom:0;">'+
												...
												'</li>'+
												'<li class="huoBi" style="margin-top:0;margin-bottom:0;">'+
													'&nbsp;<span>'+(KAIPIAOREN == '' || KAIPIAOREN == null || KAIPIAOREN == 'null'  || typeof KAIPIAOREN == 'undefined'
														? '' :KAIPIAOREN)+'</span>'+
												'</li>'+
												'<li class="huoBi2" style="margin-top:0;margin-bottom:0;">'+
													'&nbsp;<span id="paperSHOUKUANREN'+paperCount+'" style="white-space:nowrap !important;"></span>'+
					  ...
					  if(!isEnd){
						  ...
						  htmls += '<tr style="height:8mm;">'+
										...
										'<td colspan="3">'+
											'<table style="width:100%;border-collapse: collapse;table-layout: fixed;">'+
												'<tr>'+
													'<td style="width:7mm;">'+totalMoneyArray[0]+'</td>'+
													'<td style="width:7mm;">'+totalMoneyArray[1]+'</td>'+
													'<td style="width:7mm;">'+totalMoneyArray[2]+'</td>'+
													'<td style="width:8mm;">'+totalMoneyArray[3]+'</td>'+
													'<td style="width:7mm;">'+totalMoneyArray[4]+'</td>'+
													'<td style="width:7mm;">'+totalMoneyArray[5]+'</td>'+
													'<td style="width:7mm;">'+totalMoneyArray[6]+'</td>'+
												'</tr>'+
											'</table>'+
										'</td>'+
										'<td></td>'+
				...
				$("#printAreaGUANGDIANshouju2").html(htmls);
				//将每张票的收款人添加到票上
				for(var i = 0; i < SHOUKUANRENArray.length; i++){
					var SHOUKUANRENSS = SHOUKUANRENArray[i].substr(0, SHOUKUANRENArray[i].length - 1);
					var SHOUKUANRENSSArray = SHOUKUANRENSS.split("/");
				...
				//打印区域
				$("#printAreaGUANGDIANshouju").printArea();
