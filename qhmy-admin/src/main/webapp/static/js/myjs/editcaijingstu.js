...
(function($, window) {
	...
	stuinfo.save = function() {
		...
		if(zaixuezhuangtai==''||zaixuezhuangtai==null){
			layer.msg("在学状态不能为空");
			return false;
		}
		...
		var RUXUESHIJIAN = $("#RUXUESHIJIAN").val();//计划性质
		var PKID = $("#pkid").val();
		...
		var data={SHENFENZHENGHAO:sfz,CHUSHENGRIQI:csrq,XINGBIE:xingbie,XINGMING:name,SHOUJI:phone,NATION_PKID:minzu,
				  ...
				  JTCYLXDH:lxdh,JTCYGX_PKID:jtcygx,JIATINGZHUZHI:jiatingzhuzhi,XUEHAO:xuehao,POOR_PKID:poorstu,specialFlag:specialFlag,
				  ...
				  XSLX_PKID:xueshengleixing,XUEJIZHUANGTAI_PKID:xuejizhuangtai,STUDENT_SOURCE:STUDENT_SOURCE,RUXUESHIJIAN:RUXUESHIJIAN
		};
...
$(function(){
	...
	for(var i=0;i<stu_PersonalFlag1.length;i++){
		$("input[name='checkbox1']").each(function() {  			
			if($(this)[0].value==stu_PersonalFlag1[i].EXCEPTIONAL){
				$(this)[0].checked=true;
			}
		});  
	}
