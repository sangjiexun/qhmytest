...
/*		$("#poorTable").bootstrapTable('destroy');
	...
	poor.goUpdateAndAdd = function(PKID){
		...
		BootstrapDialog.show({ // 显示需要提交的表单。
			title : tit,
			message : $('<div></div>').load(_basepath+'learningSub/goEdit.json?PKID='+PKID),
			closable : true,
			buttons : [
					{
						label : '确定',
	...
	poor.updateAdds = function(dialogRef){
		...
		var IS_QYS=$("#IS_QYS").is(':checked');//是否启用
		var IS_QY = $("#IS_QY").is(':checked');//是否启用
		if(IS_QY){
			IS_QY = 'Y';
		}else{
		...
		/*	if(NAME==CFNAME&&IS_QY==IS_QYS){
			...
			$.ajax({
				...
				dataType : "json",
				async: false,
				url : _basepath+ 'learningSub/saveAndUpdate.json',
				data : {
					"PKID":PKID,
					"NAME":NAME,
					"IS_QY":IS_QY,
	...
	poor.del = function(pkid){
        ...
        closable: true, 
          buttons: [{
		    ...
		    cssClass: 'btn-danger',
		    action: function(dialogRef){
		    	 var url = _basepath + "learningSub/del.json";
	...
	poor.getTab = function() {
		//获得按钮权限
		var url = _basepath + "learningSub/list.json";
										...
										formatter:function(value,row,index){
											...
											'click .al-toggle-button':function(e,value,row,index){
												 ...
												 if(bol)
													 isqy="Y";
												 var pkid=row.PKID;
												 var url = _basepath + "learningSub/updateIsQy.json";
