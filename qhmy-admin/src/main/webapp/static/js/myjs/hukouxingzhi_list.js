 ...
 * 户口性质对象
 */
//@ sourceURL=hukouxingzhi_list.js
(function($, window) {
	var poor = {};
	...
	poor.goUpdate = function(pkid){
		BootstrapDialog.show({ // 显示需要提交的表单。
			title : '编辑户口性质',
			message : $('<div></div>').load(_basepath+'accountnature/goEdit.json?DICTIONARIES_ID='+pkid),
			closable : true,
			buttons : [
					{
						label : '确定',
          ...
          buttons: [{
		    ...
		    action: function(dialogRef){
		    	 $.ajax({
						type:"post",
						dataType:"json",
						url:_basepath+'accountnature/del.json?DICTIONARIES_ID='+pkid,
	...
	$("#btn_add").click(function(){
		...
		BootstrapDialog.show({ // 显示需要提交的表单。
			title : '新增户口性质',
			message : $('<div></div>').load(_basepath+'accountnature/goEdit.json?PKID='+pkid),
			closable : true,
			buttons : [
					{
						label : '确定',
	...
	poor.updateAdd = function(dialogRef){
		...
		if(flag){
			...
			$.ajax({
				...
				dataType : "json",
				async: false,
				url : _basepath+ 'accountnature/edit.json',
	...
	poor.validate = function(NAME,PKID){
		...
		$.ajax({
			...
			dataType : "json",
			async: false,
			url : _basepath+ 'accountnature/getpoor_NAME.json',
	...
	poor.getTab = function() {
		...
		var url = _basepath + "accountnature/accountNatureTable.json";
							...
							columns : [
									...
									},
									{
										...
										formatter : function(value, row, index) {
											if(row.IS_USED=='Y'){
												return [
												        '<input type="checkbox" checked="checked" class="al-toggle-button" '+(SESSION_MENU_BUTTONS.hkxz_qy == 1?'':'disabled="disabled"')+'>']
												.join('');
											}else{
												return [
												        '<input type="checkbox" class="al-toggle-button" '+(SESSION_MENU_BUTTONS.hkxz_qy == 1?'':'disabled="disabled"')+'>']
												.join('');
											}
										...
										events : {
											'click .al-toggle-button' : function(e,value, row, index) {
												...
												var url = _basepath + "accountnature/updateIsUse.json";
												$.ajax({
													type:"post",
													dataType:"json",
									...
									},
									{
										...
										formatter : function(value, row,index) {
											return [
													SESSION_MENU_BUTTONS.hkxz_edit == 1?'<a class="edit ml10" href="javascript:void(0)" title="修改"><i class="fa fa-pencil" ></i></a>&nbsp;&nbsp;':'',
													SESSION_MENU_BUTTONS.hkxz_del == 1?' <a class="del ml10" href="javascript:void(0)" title="删除"><i class="fa fa-trash-o" ></i></a>':''
															 ]
													.join('');
										}, //紫色为添加图标（icon），插件：font-awesome，效果图见底部。
										 events : {
