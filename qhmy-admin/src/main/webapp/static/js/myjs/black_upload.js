...
black.biz = function(){
	...
	if(black.pkidList!=null && black.pkidList.length>1){
		var data = {};
		data.pkidListStr = JSON.stringify(black.pkidList); 
		var url = _basepath+"blacklist/copyFjFiles.json";
...
//				$("#nameli").load(_basepath+"blacklist/list.json");
//			    $("#erjie").empty();
//			    $("#erjie").hide();
 ...
 * 初始化上传组件
 */
black.initUpload = function(fileObjs){
...
//			  zhubiao.remove(tempFile);
		...
		onAddTask: function(file) {
			var filename = "";
		     ...
		     '</td>' +
		     '<td><span style="cursor: pointer;" class="glyphicon glyphicon-remove" onClick="javascript:black.delFjTr(\'' + file.id + '\')"></span>' +
		     '</td></tr>';
