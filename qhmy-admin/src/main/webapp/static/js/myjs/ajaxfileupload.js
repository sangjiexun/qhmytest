					...
					try {
						if (io.contentWindow) {
							xml.responseText = io.contentWindow.document.body ? io.contentWindow.document.body.innerHTML
									: null;
							var pre = $(window.frames[frameId].document.body).find("pre");
//							alert($("#"+frameId).contents().find("body").find("pre").html());
//							alert($(frameId).contents().find("body").html());
							var preObj = $(io.contentWindow.document.body).find("pre");
							if(preObj!=null && preObj.size()>0){
								xml.responseText = preObj.html();
							}
