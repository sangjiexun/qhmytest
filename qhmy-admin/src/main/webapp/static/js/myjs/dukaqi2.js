...
dukaqi.connect = function (){
	var version = MWRFATL2.openReader(0,9600);//USB端口是0和9600  返回设备版本号  长度18字节
	//MWRFATL2.ReaderBeep(30);//控制设备蜂鸣器  ReaderBeep  30毫秒蜂鸣时间
	return version;
};

//寻卡  打开卡片
dukaqi.scard = function(){
	var cardNumber = MWRFATL2.openCard(0);//0一次只能操作一张卡片   返回四个字节的序列号    如：5FC5B768
	return cardNumber;
};
...
dukaqi.readerLoadKey = function ()
{
	...
	//
	var iRet=MWRFATL2.Loadkey(0,1,key);
	if(iRet)
	{
//		alert(iRet);
...
{
	var iRet = MWRFATL2.mifare_authentication(0,11);
	if(iRet)
	{
//		msg.value =msg.value + "验证方式一验证密码失败！" +"\n";
		iRet = MWRFATL2.mifare_authentication_2(0,11,44);//此函数验证时传入的是块号，验证成功后，这个扇区的所有块都可以读写操作
		if(iRet)
		{
...
{
	var data="科曼信息技术公司";
	var iRet = MWRFATL2.mifare_write(44,data);
	if(iRet)
	{
//		alert(iRet);
...
{
	var iRet=MWRFATL2.mifare_read(44);
//	alert(iRet);
...
dukaqi.writeDataHex = function (val)
{
	...
	var iRet = MWRFATL2.mifare_writeHex(44,val);
	if(iRet)
	{
...
{
//	alert(44);
	var databuff = MWRFATL2.mifare_readHex(44);
//	alert(databuff);
...
{
	var iRet = MWRFATL2.ChangeKey(11,"ffffffffffff",0,0,0,1,"ffffffffffff");
	alert(iRet);
	if(iRet)
	{
...
{
	var iRet = MWRFATL2.CloseCard();//关闭开篇
//	if(iRet)
//	{
...
dukaqi.ascToHex = function(){
	var asc = "12345678"+"";
	alert(asc.length);
	var bstr = MWRFATL2.ascToHex(asc, asc.length);
	alert(bstr);
	return bstr;
};
...
dukaqi.hexToAsc = function(){
	var hex = "";
	var rst = MWRFATL2.hexToAsc(hex,hex.length);
	alert(rst);
	return rst;
};
...
dukaqi.initValue = function(block,value){
	var rst = MWRFATL2.mifare_initValue(block,value);
	alert("结果:"+rst);
};
...
dukaqi.getBlockValue = function(block){
	var rst = MWRFATL2.mifare_value(block*1);
	alert("结果:"+rst);
};
...
dukaqi.increment = function(block,value){
	var b = 64;
	var v = 0;
	var rst = MWRFATL2.mifare_increment(b,v);
	alert(rst);
};
...
dukaqi.decrement = function(bolck,value){
	var b = 64;
	var v = 0;
	var rst = MWRFATL2.mifare_decrement(b,v);
	alert(rst);
};