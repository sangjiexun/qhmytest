...
ace.settings = {
	...
	sidebar_collapsed : function(sidebar, collapse , save,id) {
		if(ace.vars['very_old_ie']) return false;
...
//end  设置ace.settings对象属性及方法


//check the status of something
//检查是否支持item,val方法，支持，就调用settings中定义的方法，如：sidebar_fixed，入参为sidebar何fixed
...
// end  check the status of something
...
//初始化一个web存储功能的实例，如果浏览器支持，就使用html5的web存储特性,否则使用cookie
