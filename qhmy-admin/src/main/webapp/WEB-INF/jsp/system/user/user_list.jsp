								...
								<c:when test="${not empty userList}">
									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										...
										<tr>
											...
											<td class="center">${user.NAMEDEP}</td>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													...
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="editUser('${user.USER_ID}','${pd.DEPARTMENT_ID}');">
												...
												<div class="hidden-md hidden-lg">
													...
													<c:if test="${QX.edit == 1 || QX.del == 1 }">
														...
														<ul class="dropdown-menu dropdown-only-icon dropdown-yellow dropdown-menu-right dropdown-caret dropdown-close">
															...
															<c:if test="${QX.edit == 1 }">
															<li>
																<a style="cursor:pointer;" onclick="editUser('${user.USER_ID}','${pd.DEPARTMENT_ID}');" class="tooltip-success" data-rel="tooltip" title="修改">
																	<span class="green">
					...
					<table style="width:100%;">
						<tr>
							<td style="vertical-align:top;">
								<c:if test="${QX.add == 1 && pd.DEPARTMENT_ID!=null && pd.DEPARTMENT_ID!=''}">
								<a class="btn btn-mini btn-success" onclick="add('${pd.DEPARTMENT_ID}');">新增</a>
								</c:if>
	...
	bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
		if(result) {
			...
			$.get(url,function(data){
				...
				parent.location.href="<%=basePath%>user/listAllUserDepartment.do?DEPARTMENT_ID=${pd.DEPARTMENT_ID}&dnowPage=${page.currentPage}";
			});
		};
	});
}
...
function add(id){
	 ...
	 diag.Drag=true;
	 diag.Title ="新增";
	 diag.URL = '<%=basePath%>user/goAddU.do?DEPARTMENT_ID='+id;
	 diag.Width = 1000;
	 diag.Height = 1200;
...
//修改
function editUser(user_id,dep_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=basePath%>user/goEditU.do?USER_ID='+user_id+'&DEPARTMENT_ID='+dep_id ;
	 diag.Width = 1000;
	 diag.Height = 1200;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
//打开上传excel页面
function fromExcel(department_id){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag=true;
	 diag.Title ="EXCEL 导入到数据库";
	 diag.URL = '<%=basePath%>user/goUploadExcel.do?DEPARTMENT_ID='+department_id;
	 diag.Width = 300;
	 diag.Height = 200;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
